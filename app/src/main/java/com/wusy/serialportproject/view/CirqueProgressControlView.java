...
import com.wusy.serialportproject.util.BitmapUtils;
import com.wusy.serialportproject.util.ChartUtils;
import com.wusy.serialportproject.util.DensityUtils;
...
public class CirqueProgressControlView extends View {
    ...
    private int mMyPadding=15;//wusy因为分辨率问题，减少半径，相当于加内边距
    ...
    private int[] mArcColors = new int[]{
            ...
            Color.parseColor("#ffffff"),
            Color.parseColor("#2793ff"),
            Color.parseColor("#eb4f39"),
    };
    private float[] mArcColorsPosition=new float[]{
            0f,0f,0.5f,0.85f
    };
    ...
    private void initPaint() {
        ...
        mCircleBgPaint.setColor(Color.BLACK);
...
//        mCirqueBgPaint.setStrokeWidth(DensityUtils.dip2px(mContext, mLineStrokeWidth));
        ...
        mCirqueBgPaint.setStrokeWidth(56);
...
//        mCirquePaint.setStrokeWidth(DensityUtils.dip2px(mContext, mLineStrokeWidth));
        ...
        mCirquePaint.setStrokeWidth(56);
        ...
        mDragBitmap = BitmapUtils.getBitmap(mContext, R.mipmap.btn_change);
        mDragBitmap = BitmapUtils.conversionBitmap(mDragBitmap, DensityUtils.dip2px(mContext,
                100), DensityUtils.dip2px(mContext, 100));
        mOval = new RectF();
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mRadius = minXY - mLineStrokeWidth / 2 - mDragBitmap.getWidth() / 2-mMyPadding;
        ...
        mCirquePaint.setShader(new SweepGradient(mCenterX+50, mCenterY*2, mArcColors, mArcColorsPosition));
    ...
    private void drawCirque(Canvas canvas) {
        if (mCurrentAngle != 0) {
            canvas.drawArc(mOval, startAngle, mCurrentAngle, false, mCirquePaint);//起始点90度
        }
    }
    ...
    private void drawText(Canvas canvas) {
        ...
        canvas.drawText(mText, getWidth() / 2 - mTextPaint.measureText(mText) / 2, getHeight() / 2 +
                DensityUtils.dip2px(mContext, 20)+mMyPadding, mTextPaint);
    }
    ...
    private void updateCurrentAngle(float x, float y) {
        ...
        if (pointX >= 0 && pointY <= 0 ) {
            ...
            mCurrentAngle = (int) Math.toDegrees(atan) + 180.f;
            lastQuadrant = 1;
        }
